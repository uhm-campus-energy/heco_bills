...
import config
from heco_common import organize_bills, load_max_dates, filter_already_billed


def main():
    folder = sys.argv[1] if len(sys.argv) > 1 else str(config.SOURCE_DIR)

    print("Astronomy Bill Organizer")
    print("=" * 70)
    print(f"Working folder:\n  {folder}\n")

    max_dates = load_max_dates(config.LAST_DATES_CSV)
    filter_already_billed(
        folder_path=folder,
        duplicate_dir=config.DUPLICATE_DIR,
        max_dates=max_dates,
        get_contract_and_date=config.get_contract_and_date,
    )
